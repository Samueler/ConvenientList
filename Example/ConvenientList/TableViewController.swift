...
class TableViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        data?.tableViewDidSelectedClosure = { [weak self] tableView, indexPath in
            ...
            data.create(sectionData: section, section: data.listSections?.count ?? 0)
            
//            let result = data.create(rowData: row, at: IndexPath(row: data.listSections?[0].listRows?.count ?? 0, section: 0))
            
//            let result = data.update(sectionData: section, at: 0)
//            let result = data.update(rowData: row, at: IndexPath(item: 0, section: 0))
//            let result = data.delete(at: 0)
            
//            let result = data.delete(rowData: IndexPath(row: 0, section: 0))
